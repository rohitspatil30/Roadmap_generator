...
def generate_course_structure(field_of_study):
    prompt = f'''Act as an expert academic advisor expert in {field_of_study}. Your task is to provide the steps to pursue the {field_of_study} in terms of topics that need
    to be completed step-wise to excel in {field_of_study}. Please answer in the form of a dictionary showing which step is a prerequisite of which step to have a direction
    in the flowchart. Also, mention each and every topic to cover {field_of_study} with each step. Please keep the format as - For example, if the course is Mechanical Engineering, then the answer should be in the format-
    {{
...
if st.button("Generate Course Roadmap"):
    if field_of_study:
        ...
        try:
            ...
            st.error("Failed to parse the course structure. Please try again.")
    else:
